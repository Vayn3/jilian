    ...
    async def synthesize(self, text: str) -> AsyncGenerator[bytes, None]:
        ...
        try:
            async with websockets.connect(url, ssl=True) as ws:
                ...
                logger.debug(f"TTS发送文本: {text[:50]}...")
...
class TTSConnectionPool:
    ...
    async def initialize(self) -> None:
        ...
        logger.debug(f"TTS连接池已初始化，大小: {self.pool_size}")
...
class RealtimeTTSSession:
    ...
    async def stop(self) -> None:
        ...
        if self._task:
            ...
            try:
                ...
                pass
        logger.debug("TTS会话已停止")

    def interrupt(self) -> None:
        """打断当前合成"""
        self._interrupted.set()
        logger.debug("TTS合成被打断")
    ...
    def _detect_llm_keywords(self, text: str) -> None:
        ...
        for keyword, patterns in LLM_KWS_PATTERNS.items():
            ...
            matched_pattern = next((p for p in patterns if p in buf), None)
            if matched_pattern:
                self._udp_controller.emit_voice_keyword(keyword)
                logger.info(
                    f"[LLM-KWS] 匹配: '{matched_pattern}' → 动作: {keyword} | UDP已发送"
                )
    ...
    async def _run(self) -> None:
        ...
        while self._running:
            try:
                ...
                logger.debug(f"TTS开始合成: {text[:30]}...")

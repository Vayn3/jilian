...
from llm_client import LLMClient, RealtimeLLMSession, RAGInterface, SimpleRAG
from tts_client import TTSClient, RealtimeTTSSession, TextPreprocessor
from audio_manager import AudioCapture, AudioPlayer, RealtimeAudioPlaySession, SimpleVAD
from conversation import DialogManager, DialogState, DialogEvent, create_dialog_queues
...
class VoiceDialogSystem:
    ...
    def _init_components(self) -> None:
        ...
        self.audio_player = RealtimeAudioPlaySession(
            ...
            config=self.config.audio,
            aec_callback=self.audio_capture.update_playback_reference if self.audio_capture else None,
        )
    ...
    async def _process_audio(self, audio_data: bytes) -> None:
        ...
        logger.info(f"开始ASR识别，音频长度: {len(audio_data)} bytes")
        ...
        try:
            async with ASRClient(self.config.asr) as client:
                ...
                segment_size = int(
                    ...
                    self.config.asr.segment_duration_ms / 1000
                )
        ...
        while self._running:
            try:
                ...
                if self.dialog_manager.current_state == DialogState.SPEAKING:
                    # 简单检测播放是否结束
                    if self.tts_queue.empty() and not self.audio_player.player.is_playing:
